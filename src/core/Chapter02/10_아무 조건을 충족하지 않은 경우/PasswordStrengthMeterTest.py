...
class PasswordStrengthMeterTest(unittest.TestCase):
    ...
    def test_meetsOnlyLengthCriteria_Then_Weak(self):
        """입력텍스트가 8글자 이상이 조건만 충족하는 경우"""
        #test init

        #do

        #assert
        self.assertStrength(password="abdefghi", expMeter=PasswordStrength.약함)

    def test_meetsOnlyNumCrieria_Then_Weak(self):
        """입력텍스트가 숫자 포함 조건만 충족하는 경우"""
        ...
        #assert
        self.assertStrength(password="12345", expMeter=PasswordStrength.약함)

    def test_meetsOnlyUpperCrieria_Then_Weak(self):
        """입력텍스트가 숫자 포함 조건만 충족하는 경우"""
        ...
        #assert
        self.assertStrength(password="ABZEF", expMeter=PasswordStrength.약함)

    def test_meetsNoCrieria_Then_Weak(self):
        """입력텍스트가 숫자 포함 조건만 충족하는 경우"""
        ...
        #assert
        self.assertStrength(password="abc", expMeter=PasswordStrength.약함)